...
**여기서는 스트리밍을 쓰지 않는다.** 로컬의 `/ai/chat/stream` 은 SSE 로 조각을
흘려보내지만, Vercel Python 런타임에서 응답을 흘려보내는 것이 되는지 확인되지 않았다.
확인 전까지는 한 번에 응답하는 쪽으로 둔다. 화면은 스트리밍이 실패하면 이 경로로
넘어오므로 답변 자체는 그대로 나온다.
